...
class SQL {
  ...
  createOrUpdateDBInstance(resourceGroupName, serverName, params) {
    if (!resourceGroupName || !serverName || !params) {
      throw new Error('Provide resourceGroupName, serverName and server params');
    }

    var createPromise = this._azureRestSdk
        .loginWithServicePrincipalSecret(process.env.AZURE_CLIENT_ID, process.env.AZURE_CLIENT_SECRET, process.env.AZURE_TENANT_ID)
        .then((credentials) => {
          return new SQLManagement(credentials, process.env.AZURE_SUBSCRIPTION_ID)
                    .servers
                    .createOrUpdate(resourceGroupName, serverName, params);
        });
  ...
  createOrUpdateDatabase(resourceGroupName, serverName, databaseName, params) {
    if (!resourceGroupName || !serverName || !databaseName || !params) {
      throw new Error(' Provide resourceGroupName, serverName, databaseName, params');
    }
    ...
    var createDBPromise = this._azureRestSdk
        ...
        .then((credentials) => {
          return new SQLManagement(credentials, process.env.AZURE_SUBSCRIPTION_ID)
                    .databases
                    .createOrUpdate(resourceGroupName, serverName, databaseName, params);
        });
  ...
  deleteDatabase(resourceGroupName, serverName, databaseName) {
    if (!resourceGroupName || !serverName || !databaseName) {
      throw new Error('Provide resourceGroupName, serverName and database name');
    }
    ...
    var deleteDBPromise = this._azureRestSdk
        ...
        .then((credentials) => {
          return new SQLManagement(credentials, process.env.AZURE_SUBSCRIPTION_ID)
                    .databases
                    .deleteMethod(resourceGroupName, serverName, databaseName);
        });
  ...
  deleteDBInstance(resourceGroupName, serverName, params) {
    ...
    var deletePromise = this._azureRestSdk
        ...
        .then((credentials) => {
          return new SQLManagement(credentials, process.env.AZURE_SUBSCRIPTION_ID)
                    .servers
                    .deleteMethod(resourceGroupName, serverName, params);
        });
